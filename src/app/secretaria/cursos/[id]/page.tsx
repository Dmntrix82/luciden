...
export const metadata: Metadata = { title: "Curso — Secretaría" };

export default async function PaginaCursoSecretaria({
  params,
  searchParams,
}: {
  ...
  const puedeEditar = secretaria ? esEditorActivo(secretaria.fecha_final) : false;
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between text-sm">
        <Link
          href={`/secretaria/cursos/${id}?fecha=${fechaAnterior.toISOString().slice(0, 10)}`}
          className="text-azul-medio hover:underline"
        >
          ← Día anterior
        </Link>
        {fecha !== hoy && (
          <Link href={`/secretaria/cursos/${id}`} className="text-azul-medio hover:underline">
            Volver a hoy
          </Link>
        )}
        <Link
          href={`/secretaria/cursos/${id}?fecha=${fechaSiguiente.toISOString().slice(0, 10)}`}
          className="text-azul-medio hover:underline"
        >
          Día siguiente →
        </Link>
